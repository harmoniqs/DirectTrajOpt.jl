...
_get_module_by_name(name) = [mod for mod in reverse(Base.loaded_modules_order) if Symbol(mod) == Symbol(name)][1]
...
function _solve_madnlp(model::AbstractOptimizer; array_type=nothing)
    ...
    model.result = (isa(array_type, Nothing) ? result : MadNLPMOI.copy_result_to_cpu(result))
...
@testitem "testing MadNLP.jl solver internals (using DirectTrajOpt._solve_with_kwargs with kkt_system=MadNLP.SparseUnreducedKKTSystem, linear_solver=MadNLP.LapackCPUSolver)" begin
    ...
    DirectTrajOpt._solve_with_kwargs(prob, MadNLPSolverExt.MadNLPOptions(max_iter = 100); kkt_system=MadNLP.SparseUnreducedKKTSystem, linear_solver=MadNLP.LapackCPUSolver)
end
