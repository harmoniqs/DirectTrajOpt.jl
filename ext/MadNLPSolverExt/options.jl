...
@kwdef mutable struct MadNLPOptions{T <: Float64}
    ...
    inertia_free_tol::Float64 = 0. # Only applicable when `inertia_correction_method == :inertia_free`; see https://github.com/MadNLP/MadNLP.jl/blob/master/src/IPM/solver.jl
    default_primal_regularization::Float64 = 0. # Recently added option of indeterminate utility; has no analog in Ipopt; see https://github.com/MadNLP/MadNLP.jl/pull/573
    default_dual_regularization::Float64 = 0. # Recently added option of indeterminate utility; has no analog in Ipopt; see https://github.com/MadNLP/MadNLP.jl/pull/573

    quasi_newton_init_strategy::String = "scalar_1" # "scalar_1" | "scalar_2" | "scalar_3" | "scalar_4" | "constant"
    quasi_newton_max_history::Int = 6
    quasi_newton_init_value::Float64 = 1.0
    ...
    bound_push::Float64 = 1e-2 # (corresponds to kappa_1)
    bound_fac::Float64 = 1e-2 # should be "bound_frac" (corresponds to kappa_2)
    nlp_scaling::Bool = true # (corresponds to [:gradient => true, :none => false], with no support for :user, :equilibration)
    ...
    # Restoration options
    soft_resto_pderror_reduction_factor::Float64 = 0.9999 # soft restoration phase primal-dual error required reduction
    required_infeasibility_reduction::Float64 = 0.9 # restoration phase termination criterion

    # Line search
    obj_max_inc::Float64 = 5. # max acceptable order of magnitude increase in barrier objective
    kappa_soc::Float64 = 0.99 # 2nd order correction sufficient reduction rule factor
    max_soc::Int = 4 # 2nd order correction max trial steps per iteration
    alpha_min_frac::Float64 = 0.05 # minimal step size safety factor
    s_theta::Float64 = 1.1 # switching rule current constraint violation exponent
    s_phi::Float64 = 2.3 # switching rule linear barrrier function exponent
    eta_phi::Float64 = 1e-4 # Armijo relaxation factor; Ipopt uses default of 1e-8
    gamma_theta::Float64 = 1e-5 # constraint violation filter margin relaxation factor
    gamma_phi::Float64 = 1e-5 # barrier function filter margin relaxation factor; Ipopt uses default of 1e-8
    delta::Float64 = 1. # switching rule constraint violation multiplier
    kappa_sigma::Float64 = 1e10 # primal estimate-dual variable deviation limitation factor
    barrier_tol_factor::Float64 = 10. # barrier stop test mu factor
    rho::Float64 = 1000. # restoration penalty parameter (corresponds to IpoptOptions.resto_penalty_parameter, NOT IpoptOptions.rho)
    
    # Barrier
    barrier::String = "monotone" # "monotone" | "adaptive_quality_function" | "adaptive_loqo" # barrier update algorithm
    tau_min::Float64 = 0.99 # lower bound on fraction to boundary

    barrier_update_mu_init::Float64 = 1e-1
    barrier_update_mu_min::Float64 = 1e-11
    barrier_update_mu_superlinear_decrease_power::Float64 = 1.5
    barrier_update_mu_linear_decrease_factor::Float64 = .2
    barrier_update_mu_max::Float64 = 1e5
    barrier_update_sigma_min::Float64 = 1e-6
    barrier_update_sigma_max::Float64 = 1e2
    barrier_update_sigma_tol::Float64 = 1e-2
    barrier_update_gamma::Float64 = ((barrier == "adaptive_loqo") ? 0.1 : 1.0)
    barrier_update_max_gs_iter::Int = 8
    barrier_update_free_mode::Bool = true
    barrier_update_globalization::Bool = true
    barrier_update_n_update::Int = 0
    barrier_update_r::Float64 = .95
