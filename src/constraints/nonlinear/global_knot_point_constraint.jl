...
function CommonInterface.eval_jacobian(
    ...
    @views for (i, t) ∈ enumerate(constraint.times)
        ...
        ∂g_compact =
            ForwardDiff.jacobian(x -> constraint.g(x, constraint.params[i]), xg_data)
...
function CommonInterface.eval_hessian_of_lagrangian(
    ...
    @views for (i, t) ∈ enumerate(constraint.times)
        ...
        μ∂²g_full[knot_range, knot_range] .+=
            μ∂²g_compact[1:constraint.var_dim, 1:constraint.var_dim]

        # Knot × Global block
        μ∂²g_full[knot_range, offset_global_comps] .+=
            μ∂²g_compact[1:constraint.var_dim, (constraint.var_dim+1):end]
